...
def run_single_backtest(symbol: str = 'btc', scenario: str = 'adaptive'):
    ...
    try:
        ...
        label_str = 'DQN Neuro-Symbolic (5-Action)' if scenario_lower == 'adaptive' else 'DQN Pure Baseline (5-Action)'
        ...
        ax1.set_title(f"Equity Curve: {symbol_lower.upper()} [DQN 5-Action] (Return: {metrics['total_return']:+.2f}%, MDD: {metrics['max_drawdown']:.2f}%)", fontsize=11, fontweight='bold', pad=14)
        ...
        chart_file = os.path.join(asset_chart_dir, f"chart_{symbol_lower}_dqn_{scenario_lower}.png")
...
def run_full_benchmark_suite(symbols=('btc', 'eth', 'xrp')):
    print(f"\n{'='*70}")
    print("RUNNING MULTI-ALGORITHM BENCHMARK SUITE (DQN, PPO, SAC, HODL)")
    print(f"{'='*70}\n")
    ...
    for sym in symbols:
        ...
        # 1. 5-Action DQN
        res_dqn_nesy = run_single_backtest(sym, scenario='adaptive')
        res_dqn_base = run_single_backtest(sym, scenario='baseline')
        ...
        # 2. PPO Baseline
        res_ppo_nesy = run_single_backtest(sym, scenario='adaptive')
        res_ppo_base = run_single_backtest(sym, scenario='baseline')
        t_ppo = paired_t_test(res_ppo_nesy['returns'], res_ppo_base['returns'])
        ...
        summary_rows.append({
            ...
            't-stat': f"{t_ppo['t_statistic']:.3f}",
            'p-val': f"{t_ppo['p_value']:.4f}",
            'Sig (p<0.05)': "YES" if t_ppo['is_significant_5pct'] else "NO"
        })
    ...
    # Save Markdown
    md_path = f"{TABLES_DIR}/table5_summary_results.md"
    try:
    ...
    with open(md_path, 'w') as f:
        f.write("# Table 5: Multi-Algorithm DRL Performance & Statistical Significance\n\n")
        f.write(md_content)
    print(f"Saved summary report: {md_path}")

    # Save LaTeX
    latex_path = f"{TABLES_DIR}/table5_paper.tex"
    with open(latex_path, 'w') as f:
        f.write("% Table 5: Multi-Algorithm DRL Performance & Inferential Statistical Tests\n")
